...
void deleteNode(linkedList_h* L, listNode* p) {
	listNode* pre;					// 삭제할 노드의 앞 노드를 나타냄
	...
	if (L->head->link == NULL) {    // 리스트에 노드가 한 개만 있는 경우
		...
		pre->link = p->link;			// 원래 노드 p의 앞 노드와 뒤 노드를 연결해서 리스트 복구
		free(p);					// 삭제한 노드의 메모리 해제	 		
	}
}


listNode* searchNode(linkedList_h* L, char* x) { //노드탐색
	listNode* temp;
	temp = L->head;
	while (temp != NULL) {
...
void reverse(linkedList_h* L) { //역산연산
	listNode* p;
	listNode* q;
	listNode* r;
